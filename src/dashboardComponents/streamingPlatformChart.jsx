...
const StreamingPlatformChart = () => {
    ...
    useEffect(() => {
        const fetchStreamingPlatformData = async () => {
            try {
                ...
                console.error('Error fetching streaming platform data:', error.message);
            }
        };

        fetchStreamingPlatformData();
    }, []);
    ...
    const handleFilterClick = async () => {
        ...
        try {
            ...
            const streamingPlatformResponse = await fetch(`${apiUrl}/api/getStreamingPlatformChartData?${filterParams}`);
            const filteredstreamingPlatformData = await streamingPlatformResponse.json();

            setStreamingPlatformData(filteredstreamingPlatformData);
        } catch (error) {
    ...
    return (
        <div className="col-md-12 mb-4">
            ...
            <div className="card">
                <div className="card-body">
                    <h5 className="card-title">Most used Streaming Platform</h5>
                    <Bar
                        data={{
                            labels: streamingPlatformData.map((data) => data.label),
                            datasets: [
                                {
                                    label: '',
                                    data: streamingPlatformData.map((data) => data.count),
...
export default StreamingPlatformChart;
